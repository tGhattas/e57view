...
const AGENT_HELP = {
  ...
  post: { session: 'session id', cmd: 'state | screenshot | view | section | probe | heightmap | contour | fitplane | distance | inside | measure | pick | set_view | set | regions | entities | register | distance_to | fit | detect | volume | mesh | script | analysis | transform | surface | history | stations | open | cache', args: {} },
  ...
  examples: [
    ...
    { cmd: 'analysis', args: { op: 'sor', neighbours: 8, nSigma: 1 } },
    { cmd: 'analysis', args: { op: 'noise', neighbourhood: 'radius', radius: 0.15, nSigma: 1 } },
    { cmd: 'cache', args: { op: 'write' } },
    { cmd: 'mesh', args: { op: 'measure' } },
    { cmd: 'mesh', args: { op: 'sample', density: 400 } },
    { cmd: 'mesh', args: { op: 'distance', mesh: 'design' } },
    { cmd: 'script', args: { steps: [{ cmd: 'state', save: 's' }, { cmd: 'fitplane', args: { box: { center: '$s.bounds.local.centre', half: [2.9, 1.9, 0.03] } } }] } },
